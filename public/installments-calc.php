...
} elseif ($price > $min_value)
{
    ...
    {		
		...
		while ( $i < $installments) {
			...
			$installments_html .= "<p class='price fswp_calc'>" . sprintf(__('<span class="fswp_installment_prefix">%s %sx de </span>', 'wc-parcelas'), $prefix, $i) . wc_price($price / $i) . " <span class='fswp_installment_suffix'>" . $suffix . "</span></p>";
			$installments_html .= "</div></li>";
		}
